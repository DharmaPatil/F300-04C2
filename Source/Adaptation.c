...
//    const u32 u32MinTime[] = {50, 100, 150, 200, 250, 300, 350, 400};    // 大/中投修正等级对应的小投时间
    ...
    pfs_Adapt->s32RushWtMiddle = 0;        //过冲量中点
    pfs_Adapt->s32RushWtHigh = 0;          //过冲量高点
    pfs_Adapt->s32RushWtLowProb = 0;       //统计过冲量低点
    pfs_Adapt->s32RushWtMiddleProb = 0;    //统计过冲量中点
    pfs_Adapt->s32RushWtHighProb = 0;      //统计过冲量高点
...
{
    #define DEFAULT_SP3K    1000    //当计算K为0时 默认该值最为K值
    ...
    static s32 lss32_KRecord[SLFALLWT];
    static s32 lss32_K_Pro[SLFALLWT];
    //小投K值没有采集到 可能因为小投没有达到采集的时间 则默认用上次计算出K值
    if (fs32_KThisTime == 0)    
    {
    ...
    else if(pfs_Adapt->u32SP3KCnt>=2)
    {
         ...
         BubbleSort(lss32_K_Pro, pfs_Adapt->u32SP3KCnt + 1); //冒泡排序
    ...
    {
         if(pfs_Adapt->u32SP3KCnt==0) //如果是刚启动，清零
         {
            lss32_AddFallWTemp = 0;
         }
         lss32_KRecord[pfs_Adapt->u32SP3KCnt] = fs32_KThisTime;
...
/*********************************************************************
函数名称: SampleRushWt
功    能: 采集快加基于小投门限时间的最小、中点、最大 三个过冲重量
说    明: 移植时将该函数放在禁判时间之后
入口参数: *pfs_Adapt 自适应参数结构体
出口参数: 
...
void SampleRushWt(struct_adapt_t *pfs_Adapt)
{
    ...
    {   //小于等于禁判时间+判定修正中投的小投禁判后最小时间
        pfs_Adapt->s32RushWtLow = (*pfs_Adapt->ps32Weight) - (*pfs_Adapt->ps32SP2RealWt);//当前重量 - 中投截止点实际重量;
//        pfs_Adapt->u32SP2ResultWt = (*pfs_Adapt->ps32Weight);
    ...
    {   //小于等于禁判时间+判定修正中投的小投禁判后最小时间和最大时间的中点时间
        pfs_Adapt->s32RushWtMiddle = (*pfs_Adapt->ps32Weight) - (*pfs_Adapt->ps32SP2RealWt);//当前重量 - 中投截止点实际重量;
    }
    if ((*pfs_Adapt->pu32SP3RealTime) <= ((*pfs_Adapt->pu32SP3ForbidTime) + pfs_Adapt->u32SP2ThresholdMin + pfs_Adapt->u32SP2ThresholdVal))
    {   //小于等于禁判时间+判定修正中投的小投禁判后最大时间
        pfs_Adapt->s32RushWtHigh = (*pfs_Adapt->ps32Weight) - (*pfs_Adapt->ps32SP2RealWt);//当前重量 - 中投截止点实际重量;
    }
...
//            {
//                pfs_Adapt.s32RushWtLow = 当前重量 - 大投截止点实际重量;
//            }
...
//            {
//                pfs_Adapt.s32RushWtLow = 小投截止点重量 - 大投截止点实际重量;
//            }
//        }
//    }
}
...
//    s32 ls32_StdRushW = 0;
    ...
    if(pfs_Adapt->u32SP2Count>SLRUSHWT-1)
    {
        ...
        BubbleSort(lss32_RushWtLowPro, SLRUSHWT); //冒泡排序
        BubbleSort(lss32_RushWtMiddlePro, SLRUSHWT); //冒泡排序
        BubbleSort(lss32_RushWtHighPro, SLRUSHWT); //冒泡排序
...
/*********************************************************************
函数名称: void RushWtCompensate(struct_adapt_t *pfs_Adapt)
功    能: 过冲采集量的补偿
说    明: 过冲采集点时间没有到达就已经停止小投加料 此时对过冲量进行补偿 
          加上根据小投线性拟合K值计算的小投加料量
入口参数: 
出口参数: 
返 回 值: 
设    计: 丛海旭               时    间: 2015-08-27
修    改:                      时    间: 
*********************************************************************/
void RushWtCompensate(struct_adapt_t *pfs_Adapt)
{
    ...
    {   //小投时间小于过冲量采集点时间 进行补偿
        ...
        //根据小投统计的k值算出补偿重量 注意k值的单位是1次AD转换(3位小数点) 要换算成时间
...
u32 Adapt_SP12(struct_adapt_t *pfs_Adapt)
{
    ...
    else
    {
        ...
        else
        {
            ...
            RushWtCompensate(pfs_Adapt);    //过冲量补偿处理
            RushWtProbabilityStatistics(pfs_Adapt);  // 对过冲量进行概率统计处理
            ...
            ls32_InterestWeight = pfs_Adapt->s32RushWtMiddleProb;   //使用时间中点过冲值判断
...
//                        ls32_Temp = ModifyRushWeight(*pfs_Adapt->ps32SP2 - ls32_Temp, pfs_Adapt);
                        ...
                        //中投调整时间 = 过冲点调整量 / 中投流量
                        //中投提前量调整结果 = 中投调整时间 × 中投线性拟合K(转换成ms单位)
                        ...
                        pfs_Adapt->u32SP2Count = 0; //修正后重新计算过冲量
                    }
