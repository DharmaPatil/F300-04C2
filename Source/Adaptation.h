...
typedef struct
{
    ...
    s32 s32RushWtLow;           //过冲量低点
    s32 s32RushWtMiddle;        //过冲量中点
    s32 s32RushWtHigh;          //过冲量高点
    s32 s32RushWtLowProb;       //统计过冲量低点
    s32 s32RushWtMiddleProb;    //统计过冲量中点
    s32 s32RushWtHighProb;      //统计过冲量高点
    ...
    u32 u32SP3KCnt;             //统计处理小投线性拟合K值用的计数变量
    s32 s32SP2KProb;            //统计处理得到的中投截止点附近线性拟合K值
    u32 u32SP2WT;               //中投加的重量
    float fSP2Flux;             //中投流量 单位是AD次数
    u32 u32SP2ResultWt;         //中投过冲采集点时的重量
